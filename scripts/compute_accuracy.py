...
#def image_accuracy(pred, target)
...
for pred_idx, path in tqdm(enumerate(glob.iglob(os.path.join(rootdir, 'preds/hmap_per_class*.pkl')))):
    ...
    # calc distance
    #  find index with highest activation
    #  load corresponding target and pred coordinates
    #  calc distance
...
# save as txt
np.savetxt(os.path.join(rootdir, 'error_sample_mean.txt'), meanax0, delimiter=',', fmt='%.5f')
np.savetxt(os.path.join(rootdir, 'error_ldmk_mean.txt'), meanax1, delimiter=',', fmt='%.5f')
#np.savetxt(os.path.join(rootdir, 'error_mean.txt'), error_matrix.mean(), delimiter=',', fmt='%.5f')
